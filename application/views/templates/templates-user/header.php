            ...
            <div class="collapse navbar-collapse" id="navbarNavAltMarkup">
                <div class="navbar-nav">
                    ...
                    <?php if (!empty($this->session->userdata('email'))) { ?>
                        <a class="nav-item nav-link" href="<?= base_url('booking'); ?>">Booking Buku<b><?= $this->ModelBooking->getDataWhere('temp', ['email_user'  => $this->session->userdata('email')])->num_rows();
                        ?></b> Buku </a>
                        <a class="nav-item nav-link" onclick="information('Waktu Pengambilan Buku 1x24 jam dari Booking!!!')" 
                        href="<?= base_url() . 'booking/exportToPdf/' . $this->session->userdata('id_user'); ?>">Cetak Bukti Booking</a>
